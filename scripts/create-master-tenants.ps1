...
$masterTenantData = @{
    ...
    name = "Daniel Garcia"
    phone = "+5511999999999"  # Você pode ajustar para seu telefone real
...
try {
    ...
    if ($statusCode -eq 409) {
        Write-Host "⚠️  Tenant Master já existe (email já cadastrado)" -ForegroundColor Yellow
...
# ========================================
# 2. TENANT ZERO - ALTATECH IMÓVEIS
# ========================================
Write-Host "2. Criando Tenant Zero: ALTATECH Imóveis..." -ForegroundColor Yellow

$tenantZeroData = @{
    email = "administracao@altatechimoveis.com"
    password = "`$%&AltatechImoveis`$%&"
    name = "Administração"
    phone = "+5511988888888"  # Você pode ajustar para o telefone da imobiliária
    tenant_name = "ALTATECH Imóveis"
} | ConvertTo-Json
...
try {
    ...
    $statusCode = $_.Exception.Response.StatusCode.value__
    if ($statusCode -eq 409) {
        Write-Host "⚠️  Tenant Zero já existe (email já cadastrado)" -ForegroundColor Yellow
        Write-Host "   Tentando fazer login..." -ForegroundColor Gray
...
Write-Host "⚠️  ATENÇÃO: Metadados adicionais precisam ser configurados manualmente:" -ForegroundColor Yellow
Write-Host ""
...
Write-Host "📋 TENANT ZERO (ALTATECH Imóveis):" -ForegroundColor Cyan
...
Write-Host "   - settings.business_name: 'ALTATECH Imóveis'" -ForegroundColor White
Write-Host ""

Write-Host "📋 VINCULAR IMÓVEIS EXISTENTES:" -ForegroundColor Cyan
Write-Host "   Executar script para migrar imóveis sem tenant_id para:" -ForegroundColor Gray
...
# ========================================
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host "✅ RESUMO DA CRIAÇÃO" -ForegroundColor Cyan
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "🏢 TENANT ZERO - ALTATECH Imóveis" -ForegroundColor Green
Write-Host "   Email: administracao@altatechimoveis.com" -ForegroundColor Gray
